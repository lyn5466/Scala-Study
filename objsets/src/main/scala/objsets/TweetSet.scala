...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses? Abstract! (Lynn)
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    right.filterAcc(p, left.filterAcc(p, temp))
  }
  
  def filter(p: Tweet => Boolean): TweetSet = 
  	//right.filterAcc(p, left.filterAcc(p, filterAcc(p, new Empty)))
    filterAcc(p, new Empty())
  	
  def union(that: TweetSet): TweetSet = 
    //that.union(left).union(right).incl(elem)
    filterAcc(contains, that)
  
  def mostRetweeted: Tweet = {
    def douCompare(first: Tweet, second: Tweet): Tweet = 
      if (first.retweets > second.retweets) first else second
    
    if (left.isEmpty && right.isEmpty) elem
    else if (!right.isEmpty && !left.isEmpty) douCompare(right.mostRetweeted, douCompare(elem, left.mostRetweeted))
    else if (!right.isEmpty) douCompare(elem, right.mostRetweeted)
    else douCompare(elem, left.mostRetweeted)
  }
...
object GoogleVsApple {
  ...
  def tweetsMentioning(dictionary: List[String]): TweetSet =
    TweetReader.allTweets.filter(tweet => dictionary.exists(word => tweet.text.contains(word)))
...
object Main extends App {
  ...
  //val (trending, unionTime) = timeIt(GoogleVsApple.googleTweets.union(GoogleVsApple.appleTweets))
  ...
  //println(s"create union(our union): $unionTime")
  // Print the trending tweets
